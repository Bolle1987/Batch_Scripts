# VS Code with API Keys

Starts Visual Studio Code with Anthropic and OpenAI API keys provided as process-local environment variables.
...
When VS Code is started, the keys are temporarily provided through these process-local environment variables:
...
VS Code inherits these variables when it starts.

After VS Code has been launched, the variables and plaintext key values are removed from the PowerShell process.
...
However, VS Code receives its own copy of the environment when it starts and retains the API keys for its entire lifetime.

**Any process running with sufficient access under the same Windows user account may potentially read environment variables belonging to VS Code or its child processes.**

DPAPI protects the keys at rest. It does not protect them while they are actively being used by VS Code.
